...
class PlaywrightComputer(BaseComputer):
  ...
  async def _ensure_browser_is_running(self):
    # The browser can be closed by the user, in which case we need to restart it.
    # We can check if the page is closed to see if the browser is still running.
    try:
      if not hasattr(self, "_page") or self._page.is_closed():
        await self.close(None, None, None)
        await self.initialize()
    except Exception:
      # This can happen if the browser is closed and self._page is not a valid
      # page object.
      await self.close(None, None, None)
      await self.initialize()
